 ...
 * 公開ページと管理画面プレビューの両方から使う（"use client"）。
 * highlightSections を渡すと、該当セクションを色枠で囲って強調する（プレビューの変更箇所表示用）。
 */
export function LiveDetailBody({
  live,
  highlightSections,
}: {
  live: Live
  ...
  // プレビュー用：変更のあったセクションを色枠で強調（レイアウトに影響しない outline）。
  const hl = (key: string) =>
    highlightSections?.includes(key)
      ? " outline outline-2 outline-offset-4 outline-amber-500 rounded-xl"
      : ""

  // ── 各セクションを変数化（variant ごとに配置を変える）──
  const heroSection = (
    <section className={hl("hero").trim()}>
      {/* KV はトリミングせず本来の比率を維持。
  ...
  const membersSection =
    members.length > 0 ? (
        <section className={hl("members").trim()}>
  ...
  const scheduleSection = hasVenue ? (
        <section className={hl("venue").trim()}>
  ...
  const venueInfoSection = hasVenue ? (
        <section id="sec-venue" className={`scroll-mt-24${hl("venue")}`}>
  ...
  const ticketSection = hasTicket ? (
        <section id="sec-ticket" className={`scroll-mt-24${hl("ticket")}`}>
  ...
  const goodsSection = hasGoods ? (
        <section id="sec-goods" className={`scroll-mt-24${hl("goods")}`}>
  ...
  const fcSection = hasFc ? (
        <section id="sec-fc" className={`scroll-mt-24${hl("fc")}`}>
  ...
  const reportSection = hasReport ? (
          <section id="sec-report" className={`scroll-mt-24${hl("report")}`}>
  ...
  const videoSection = hasVideos ? (
        <section id="sec-video" className={`scroll-mt-24${hl("video")}`}>
